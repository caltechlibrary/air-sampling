...
source = AjaxDataSource(
    data_url=f"https://z44g6g2rrl.execute-api.us-west-2.amazonaws.com/test/phoenix/{timestamp}.json",
...
# Workaround bad default in upload script
...
slider = DatetimeSlider(
    start=start, end=end, value=end, step=600000, title="Time of measurement"
)
callback = CustomJS(
...
# Color mappers
color_mapper_pm10 = CategoricalColorMapper(
    factors=list(epa_colors.keys()), palette=list(epa_colors.values())
)
color_mapper_pm25 = CategoricalColorMapper(
    factors=list(epa_colors.keys()), palette=list(epa_colors.values())
)
...
category_legend = Legend(
    items=legend_items, title="      PM₂₋₅ (µg/m³)   PM₁₀  (µg/m³)"
)
